...
export function HeaderShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    ...
    >
      <div className="mx-auto flex max-w-full items-center gap-10 rounded-none border border-transparent bg-transparent py-2 pr-2 pl-7 transition-[max-width,border-radius,background-color,border-color,box-shadow,padding,backdrop-filter] duration-500 ease-[cubic-bezier(0.16,1,0.3,1)] group-data-[scrolled=true]/header:max-w-content group-data-[scrolled=true]/header:rounded-[28px] group-data-[scrolled=true]/header:border-white/10 group-data-[scrolled=true]/header:bg-[#0A0B0F]/85 group-data-[scrolled=true]/header:py-1.5 group-data-[scrolled=true]/header:shadow-[0_12px_40px_-12px_rgba(0,0,0,.7)] group-data-[scrolled=true]/header:backdrop-blur-xl motion-reduce:transition-none lg:pl-10">
        {children}
      </div>
    </header>
  );
